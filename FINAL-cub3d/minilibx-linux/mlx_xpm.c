...
void	*mlx_int_parse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		...
		while (i--)
		{
				...
				if ((data.data.RGB_Col = mlx_int_get_text_rgb(tab[j], tab[j+1]))==-1)
				{
				...
				if (method)
						colors_direct[mlx_int_get_col_name(line,cpp)] =
								data.data.RGB_Col>=0?mlx_get_color_value(xvar, data.data.RGB_Col):data.data.RGB_Col;
				else
				{
						colors[i].name = mlx_int_get_col_name(line,cpp);
						colors[i].col = data.data.RGB_Col>=0?mlx_get_color_value(xvar,data.data.RGB_Col):data.data.RGB_Col;
				}
				free(tab);
		}
